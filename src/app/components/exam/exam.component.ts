...
export class ExamComponent implements OnInit {
  ...
  constructor(private formBuilder:FormBuilder,private courseService:CourseService
    ,private examService:ExamService,public dialog: MatDialog,private snackBar:MatSnackBar) { }
  ...
  onSubmitExam(examForm:any){
    ...
    examForInsert.course=this.courses.filter(course=> {return course.id==examForm.courseId})[0]
  ...
  onAddExam(){
    ...
    this.courseService.getCourses().subscribe(
      data=> {
        this.courses=data.filter(course=> {return course.deleted==false})
